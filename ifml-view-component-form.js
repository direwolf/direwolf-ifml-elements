...
export class IFMLViewComponentForm extends ModelShapeRect {
  ...
  createSVGElement(viewport) {
    ...
    this.stereotypeNode = group.plain('«Form»').font({'family': 'monospace'}).attr({y:((this._height / 2) - 5), 'text-anchor': 'middle'}).cx(this._width / 2);

    this.titleNode = group.plain(this._title).font({'family': 'monospace'}).attr({y:((this._height / 2) + 10), 'text-anchor': 'middle'}).cx(this._width / 2);
  ...
  _resize() {
    ...
    this.stereotypeNode.cx(width / 2);
  ...
  _updateTitle() {
    this.titleNode.plain(this.title).font({'family': 'monospace'}).attr({y:((this.height / 2) + 10), 'text-anchor': 'middle'}).cx(this.width / 2);
  }
